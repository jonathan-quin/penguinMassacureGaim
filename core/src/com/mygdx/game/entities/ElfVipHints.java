...
public class ElfVipHints extends Node implements TimeRewindInterface {
    ...
    public ElfVipHints init(float x, float y, Texture texture){
        init(x,y);
    ...
    public void update(double delta){
        ...
        Vector2 newPos  = ObjectPool.getGarbage(Vector2.class).set(position);

        try{
            newPos = ObjectPool.getGarbage(Vector2.class).set(getGroupHander().getNodesInGroup("VIP").get(0).position);
